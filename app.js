...
// <Img>.src from our querySelector.
...
// foreach object, object.keys.
    ...
    layers.forEach(layers => {                      //Not the Value, But Keys
        Object.keys(layers).forEach(key =>{  // Object.keys(Returns An Array of "Keys" from object)
            ...
            ctx.drawImage(
                tilesetImage,
                tilesheetX * sizeOfCrop, tilesheetY * sizeOfCrop, // Start position of Source (x,y)
                sizeOfCrop, sizeOfCrop,          // size width,height of source to Grab
                positionX * sizeOfCrop, positionY * sizeOfCrop, // Where to place on canvas
                sizeOfCrop, sizeOfCrop       // WxH of Size You want it on canvas-> can stretch
            )
        })
    })
}
...
function addTiles(mouseEvent) {
    var clicked = getCoords(event);       // [x,y] of tile that was [clicked event]
